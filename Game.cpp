...
int main() {
	window.create(VideoMode(SCREEN_WIDTH, SCREEN_HEIGHT), "Breakout");
...
void load_level_one() 
{
	...
	{
		Paddle* paddle = new Paddle(SCREEN_WIDTH, SCREEN_HEIGHT, speed.x * 1.8, lives_left, i);
		Paddles.push_back(paddle);
		SceneGraph.push_back(paddle);
	}
	Ball* ball = new Ball(SCREEN_WIDTH, SCREEN_HEIGHT, 10, Paddles, speed);
	Score* score = new Score(current_score, lives_left, SCREEN_WIDTH, SCREEN_HEIGHT);
	for (float i = SCREEN_HEIGHT * .1, k = 2; i < SCREEN_HEIGHT * .4; i += SCREEN_HEIGHT * .1, k--) 
	{
...
void load_level_two() 
{
	...
	for (float i = SCREEN_HEIGHT * .2, k = 2; i <= SCREEN_HEIGHT * .4; i += SCREEN_HEIGHT * .2, k--) 
	{
...
void load_level_three() 
{
	...
	{
		if ((int)k % 2)
			b = new Brick(true, false, j, i, SCREEN_WIDTH, SCREEN_HEIGHT, 3, ball, score);
		else
